...
private:
    ...
    void parse_(const json_object & jobj)
    {
        ...
        if (const auto it = jobj.find("latency"); it != jobj.end()) {
            latency_ = JSON_CAST(JSON_IT_VALUE(it), uint32_t);
        }

        if (const auto it = jobj.find("pipelined"); it != jobj.end()) {
            pipelined_ = JSON_IT_VALUE(it).template get<bool>();
        }

        if (const auto it = jobj.find("serialize"); it != jobj.end()) {
            serialize_ = JSON_IT_VALUE(it).template get<bool>();
        }

        if (const auto it = jobj.find("rob_group"); it != jobj.end()) {
            mavis::StringListType slist = JSON_CAST(JSON_IT_VALUE(it), mavis::StringListType);
...
//        if (jobj.find("issue") != jobj.end())
//        {
//            const auto itr = issue_target_map_.find(jobj.at("issue").as_string());
//            if (itr == issue_target_map_.end())
//            {
//                throw uArchInfoUnknownIssueTarget(boost::json::value_to<std::string>(jobj.at("mnemonic")), boost::json::value_to<std::string>(jobj.at("issue")));
//            }
//            issue_target_ = itr->second;
//        }
//
...
//        if (jobj.find("unit") != jobj.end())
//        {
//            mavis::UnitNameListType ulist = boost::json::value_to<mavis::UnitNameListType>(jobj.at("unit"));
//            for (const auto & u : ulist)
//            {
//                const auto itr = umap_.find(u);
//                if (itr == umap_.end())
