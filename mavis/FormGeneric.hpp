...
public:
    FormGeneric(const json& inst, const InstMetaData::PtrType& meta)
    {
        ...
        if (const auto it = inst.find("sources"); it != inst.end()) {
            OperandNameListType olist = JSON_CAST(JSON_IT_VALUE(it), OperandNameListType);
        ...
        if (const auto it = inst.find("specials"); it != inst.end()) {
            SpecialFieldNameListType slist = JSON_CAST(JSON_IT_VALUE(it), SpecialFieldNameListType);
